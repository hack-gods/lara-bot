...
phrases = [
    ...
    'É possível que esteja com COVID-19. Procure um medico e fique em isolamento domiciliar$OK', #3
...
@app.route("/")
def index():
    return jsonify(first_lara_message())


@app.route('/<message>')
def get_message_and_return_json(message):
    response = get_user_message(message)
